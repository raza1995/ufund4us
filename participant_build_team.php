...
if (isset($_GET['cid'])) {
	$cid = $_GET['cid'];
	$uid = $_SESSION['uid'];
	$chk_participant = $oCampaign->chk_participant_campaign($cid, $uid);
	if ($chk_participant > 0) {
		...
		if (isset($_GET['pid']) && isset($_GET['did']) && $_GET['action'] == 'adddonor') {
			...
			//echo __line__.", Before direct";
			$oregister->redirect('participant_build_team.php?cid=' . $cid . '&uid=' . $pid . '');
		}
		...
		if (array_key_exists('donoredited', $_POST)) {
			...
			if (emailIsValid($pemail) || phoneIsValid($pphone)) {
				...
				if (emailIsValid($pemail)) {
					$oCampaign->send_email_to_updatedonor($cid, $pid, $did);
					//End SpartPost Send
				}
				//echo __line__.", Before direct";die();
				$oregister->redirect('participant_build_team.php?cid=' . $cid . '&uid=' . $pid . '');
			} else {
				if ($pemail != '') {
					$data['fname'] = $pname;
		...
		if (isset($_GET['pid']) && isset($_GET['cid']) && isset($_GET['did']) && isset($_GET['hash']) && isset($_GET['action']) && $_GET['action'] == 'sms_resent') {
			...
			//Donors Details
			$aDonorDetailRow = $oCampaign->getdonordetails($cid, $pid, $did);
			$donor_fname = $aDonorDetailRow['uname'];
			$donor_lname = $aDonorDetailRow['ulname'];
			$donor_phone = $aDonorDetailRow['uphone'];
			$donor_email = $aDonorDetailRow['uemail'];
			//Participant Details
			$participantid = $_SESSION['uid'];
			$participantname = $_SESSION['uname']." ".$_SESSION['ulname'];
			
			$generate_short_link = sHOME.'l.php?v='.$hash.'&u='.$participantid.'&d='.$did.'';
			...
			if ($donor_phone != "" && $donor_phone != "000-000-0000" && $donor_phone != "___-___-____") {
				if ($sent_sms = send_sms(str_replace("-","",$donor_phone),utf8_encode($body))) {
					...
					$oCampaign->sms_resend_donors($cid, $pid, $did, $donor_fname, $donor_lname, $donor_phone, $donor_email, $participantid, $participantname, $sms_sent_id, $sms_date_created, $sms_details);
				} else {
		...
		if (array_key_exists('addtolist3', $_POST)) {
			...
			$uid = $_SESSION['uid'];
			$cid = $_GET['cid'];
			// echo '<pre>'; print_r(['cid'=>$cid, 'uid'=>$uid]);die();
			$using = 'Single';
			$participantid = $_SESSION['uid'];
			$participantname = $_SESSION['uname'] . " " . $_SESSION['ulname'];
			$invalidemails = [];
			foreach ($pname as $key => $name) {
				if ($pname[$key] != '' && (emailIsValid($pemail[$key]) || phoneIsValid($phone[$key]))) {
					...
					// echo __line__.'--<pre>'; print_r(['key'=>$key, 'pemail'=>$pemail[$key],'phone'=>$phone[$key], 'aUserDetail'=>$aUserDetail);die();
					$getid = isset($aUserDetail['fld_uid']) ? $aUserDetail['fld_uid'] : '';
					if ($getid != '') {
						//echo "$cid, $getid, $uid, sanitize($name), sanitize($plname[$key]), $pemail[$key], $phone[$key], $using, $participantid, $participantname";
			...
			if ($start_date >= 0 && $start_date <= $startend_date && $camp_status == 1) {
				...
				//$oCampaign->send_email_to_manager($cid, '');
			} else {
				$oCampaign->makeitlive($cid, '', '0', $camphash);
			}
			//$oCampaign->send_email_to_donors($cid, $pid);
			// echo __line__.", Before direct";die();
			$suc_msg = 'Donor list has been successfully updated and emailed.';
			// echo $suc_msg; die();
			$oregister->redirect('participant_build_team.php?cid='.$cid.'&uid='.$pid.'&msg='.$suc_msg);

			// echo '<div id="alerttop" class="myadmin-alert myadmin-alert-icon myadmin-alert-click alert6 myadmin-alert-top" style="display: block;"> <i class="ti-user"></i>'.$suc_msg.'</div>';
		}
		...
		if (array_key_exists('golive', $_POST)) {
			...
			$phone = $_POST['phone'];
			$uid = $_GET['uid'];
			//$chkdate = $oCampaign->chkcampaign($cid, '');
			//$camp_status = $chkdate['fld_status'];
			//$start_date = $chkdate['daysstart'];
			//$end_date = $chkdate['daysend'];
			//$startend_date = $chkdate['startenddate'];
...
*/			
		...
		$styleapplied = isset($styleapplied) ? $styleapplied : '';
        ...
        <head>
            ...
            <title>Admin<?php echo sWEBSITENAME;?> - Enter Your Donors</title>
            <!-- Bootstrap Core CSS -->
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-sm-12 white-box">
                            <!-- .white-box -->
                            <div style="position: fixed; z-index: 11; width: 134px; height: 70px; top: 70px; right: 15px;text-align: center;">
                            	<a href="javascript:void(0);" data-toggle="modal" data-target="#myModal">
                            		<img src="../images/Help icon.jpg" width="60px" /><br>Need help click here
                            	</a>
                            </div>
                            
                            <h1 class="h1styling">Enter Your Donors</h1>
                            <div class="line3"></div>
                                                                ...
                                                                <div class="tabdiv">
                                                                    <div class="rowdiv">
                                                                        <table class="tab3">
                                                                            ...
                                                                            <?php
$iCountRecords = isset($iCountRecords) ? $iCountRecords : 0;
if ($iCountRecords > 0) {
				$appliedclass1 = '';
			} else {
			...
			?>
                                                                            ...
                                                                            <?
			$aDonorDetailSelected = isset($aDonorDetailSelected) ? $aDonorDetailSelected : [];
				...
				for ($i = 0; $i < $iCountRecords; $i++) {
					...
					?>
                                                                                    <tr class="itemRow">
                                                                                        ...
                                                                                        <td><?=$receiptemail;?></td>
                                                                                        <td>
                                                                                            <?if ($sent_email == 0 && $donation_amount == 0) {?>
                                                                                                <a href="?cid=<?=$cid;?>&pid=<?=$_GET['uid'];?>&did=<?=$donor_sel_id;?>&action=remove">
                                                                                                    <span class="fa fa-times" aria-hidden="true" data-toggle="tooltip" data-placement="top" data-original-title="Remove donor from campaign"></span>
                                                                                                </a>
                                                                                            <?}?>
                                                                                            ...
                                                                                            <?if ($fld_startdate <= $current_date && $fld_comdate >= 0) {?>
                                                                                                <a href="?cid=<?=$cid;?>&pid=<?=$_GET['uid'];?>&did=<?=$donor_sel_id;?>&action=resent">
                                                                                                    <span class="fa icon-envelope" data-toggle="tooltip" data-placement="top" data-original-title="Resend donation email" style="background: #9e9e9e;padding: 7px;color: #FFF;border-radius: 50%;font-size: 15px;width: 30px;height: 30px;text-align: center;margin: 0 10px 0 0;" aria-hidden="true"></span>
                                                                                                </a>
                                                                                            <?}?>

                                                                                            <?if ($fld_startdate <= $current_date && $fld_comdate >= 0) {
						//	if ($DonorPhone != "" && $DonorPhone != "000-000-0000" && $DonorPhone != "___-___-____" && $is_SMS_Allow == true) {
						?>
																							<? if ($fld_text_messaging == 1) { ?>
			...
			if ($invalidemailscount - 1 == 0 || $donorscount > 0) {
				?>
                                                                ...
                                                                <div class="tabdiv">
                                                                    <div class="rowdiv">
                                                                        <table class="tab2">
                                                                            ...
                                                                            <tbody id="inputadd3">
                                                                            <tr>
                                                                                <td><input type="text" name="pname[]" id="pname1" class="formdivtext5" placeholder="Donor First Name"></td>
                                                                                <td><input type="text" name="plname[]" id="plname1" class="formdivtext5" placeholder="Donor Last Name"></td>
                                                                                <td><input type="email" name="pemail[]" id="pemail1" class="formdivtext5 checkemail" placeholder="Donor Email"></td>
                                                                            ...
                                                                            <tr>
                                                                                <td><input type="text" name="pname[]" id="pname2" class="formdivtext5" placeholder="Donor First Name"></td>
                                                                                <td><input type="text" name="plname[]" id="plname2" class="formdivtext5" placeholder="Donor Last Name"></td>
                                                                                <td><input type="email" name="pemail[]" id="pemail2" class="formdivtext5 checkemail" placeholder="Donor Email"></td>
                                                                            ...
                                                                            <tr>
                                                                                <td><input type="text" name="pname[]" id="pname3" class="formdivtext5" placeholder="Donor First Name"></td>
                                                                                <td><input type="text" name="plname[]" id="plname3" class="formdivtext5" placeholder="Donor Last Name"></td>
                                                                                <td><input type="email" name="pemail[]" id="pemail3" class="formdivtext5 checkemail" placeholder="Donor Email"></td>
                                                ...
                                                <!-- Section 4 -->
                                                <div class="accordion_in acc_active divpad" id="previousdonors">
                                                    <div class="acc_head">Select Previous Donors</div>
                                                    <div class="acc_content">
                                                        <div class="scrol">
                                                            <div class="tabdiv">
                                                                <div class="rowdiv">
                                                                    <div class="div8 col-lg-12 col-md-12 col-xs-12">
                                                                        <div class="col-md-4">
                                                                            <input type="text" class="formdivtext4" id="fld_search" name="fld_search" placeholder="Donors Search">
                                                                        </div>
                                                                    ...
                                                                    <table class="tab1">
                                                                        ...
                                                                        <?
			$iCountRecords = count($aDonorDetail);
			if ($iCountRecords > 0) {
				for ($i = 0; $i < $iCountRecords; $i++) {
					$donor_name = $aDonorDetail[$i]['uname'];
					$donor_lname = $aDonorDetail[$i]['ulname'];
					$donor_email = $aDonorDetail[$i]['uemail'];
					$donor_phone = $aDonorDetail[$i]['uphone'];
					$donor_id = $aDonorDetail[$i]['uid'];
					?>
                                                                                <tr class="selectcheck">
                                                                                    <td><input type="checkbox" value="<?=$donor_id;?>" name="<?=$donor_name;?>" lname="<?=$donor_lname;?>" email="<?=$donor_email;?>" phone="<?=$donor_phone;?>" class="checkbox_div checkboxes" /></td>
                                                                                    <td><?=html_entity_decode($donor_name);?></td>
                                                                                    <td><?=html_entity_decode($donor_lname);?></td>
                                                                                    <td><?=$donor_email;?></td>
                                                                                    <td><?=$donor_phone;?></td>
                                                                                    <td>
                                                                                        <a href="?cid=<?=$_GET['cid'];?>&pid=<?=$uid;?>&did=<?=$donor_id;?>&action=adddonor">
                                                                                            <span class="fa fa-chevron-right" style="background: #9e9e9e;padding: 7px;color: #FFF;border-radius: 50%;font-size: 15px;width: 30px;height: 30px;text-align: center;margin: 0 10px 0 0;" aria-hidden="true" title="Add to donor"></span>
                                                                                        </a>
                                                                                        <a href="?cid=<?=$_GET['cid'];?>&pid=<?=$_GET['uid'];?>&did=<?=$donor_id;?>&action=delete">
                                                                            ...
                                                                            <?}} else {?>
                                                                            <tr>
                                                                                <td colspan="6" align="center">There is no any Donors</td>
                                                                            </tr>
                                                                        <?}?>
                                                                        </tbody>
                                                                    </table>
                                        ...
                                        <div class="col-lg-12 ">
                                            ...
                                            <div class="col-sm-6 basic-but-right" align="right">
                                                ...
                                                <button class="btn btn-success waves-effect waves-light" type="button" id="golive1" name="golive1">Update <span class="btn-label forright-icon"><i class="fa fa-chevron-right"></i></span></button>
                                            </div>
                                        </div>
                                    </div>
        ...
        <script>
            ...
            $(document).on('click', '#addmore3', function() {
                ...
                var insertnewfield = '<tr><td><input type="text" name="pname[]" id="pname'+counter+'" class="formdivtext5" placeholder="Donor First Name"></td><td><input type="text" name="plname[]" id="plname'+counter+'" class="formdivtext5" placeholder="Donor Last Name"></td><td><input type="email" name="pemail[]" id="pemail'+counter+'" class="formdivtext5 checkemail" placeholder="Donor Email"></td><td><input type="text" name="pphone[]" id="pphone'+counter+'" class="formdivtext5 checkphone" data-inputmask="'+masked+'" data-mask="" placeholder="___-___-____"></td></tr>';
            ...
            /*$('#golive').click(function(){
                var textlive = '<?if ($fld_live == 1) {echo "An email will be sent to the new donors you have just added to your campaign";} else {echo "This will make the campaign live and information about the campaign will be sent to the donors";}?>';
    swal({
		...
		<script src="//cdnjs.cloudflare.com/ajax/libs/jeditable.js/2.0.6/jquery.jeditable.js"></script> 
