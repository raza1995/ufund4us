...
$REQUEST['action'] = xss_clean($REQUEST['action']);
$REQUEST['cid'] = xss_clean($REQUEST['cid']);
$REQUEST['cuid'] = xss_clean($REQUEST['cuid']);
$REQUEST['name'] = xss_clean($REQUEST['name']);
$REQUEST['password'] = xss_clean($REQUEST['password']);
$REQUEST['redirecturl'] = xss_clean($REQUEST['redirecturl']);

$error = '';
$action = '';
if ( $REQUEST['action'] == 'joincampaign') {
	$action = 'joincampaign';
	$cid = $REQUEST['cid'];
	$cuid = $REQUEST['cuid'];
}

if (isset($REQUEST['redirecturl']) && $REQUEST['redirecturl'] != "" ) {
	$refurl1 = $REQUEST['redirecturl'];
	$refurl2 = $REQUEST['cid'];
...
if(isset($REQUEST['_hidCheckSubmit']) && $REQUEST['_hidCheckSubmit'] == "1")
{
	$pLogin = 0;
    ...
    // echo "<pre>"; var_dump($chk_multi_credential); die();
      ...
      // echo "<pre>"; var_dump($chklogin); die();
          ...
          checkAndSetInArray($chklogin, 'fld_ftime', '');
					if ($chklogin['fld_ftime'] == 1) {
						if (isset($redirecturl) && $redirecturl != '') {
							//$oregister->redirect($redirecturl);
...
<div class="divin">
   ...
   <? if ($REQUEST['action'] == 'joincampaign' && isset($REQUEST['cid']) && isset($REQUEST['cuid'])) { ?>
