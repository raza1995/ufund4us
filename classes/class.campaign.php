...
class CAMPAIGN {
	...
	public function donations_list($cid) {
		try {
			...
			FROM tbl_donations a
			LEFT JOIN tbl_campaign b ON b.fld_campaign_id = a.cid
			LEFT JOIN tbl_participants c ON c.cid = a.cid AND c.uid = a.refferal_by
	...
	public function participant_enrolled2($cid) {
		try {
			$sql = "SELECT a.uid, a.uname, a.ulname, b.fld_donor_size AS donorrequire,  up.fld_image, aa.is_unsubscribe,
			(SELECT COUNT(e.puid) FROM tbl_donors_details e WHERE e.cid = a.cid AND e.puid = a.uid) AS donorupload,
	...
	public function getparticipantdetailselected($id, $uid, $roleid) {
		...
		try {
			if ($roleid == 1 || $roleid == 3 || $roleid == 6) {
				$sql = "SELECT a.sms_sent_id, a.id, a.pid, a.uid, a.uname, a.ulname, a.uemail, a.uphone, a.is_unsubscribe, up.fld_image,
	...
	public function insert_campaign($iUid, $sName, $sLName, $sDob, $sSsn, $sEmail, $sPhone, $sAddress, $iCountryId, $iStateId, $iCityId, $sZipcode) {
		try {
			...
			$sql = "INSERT INTO tbl_campaign(fld_uid,fld_cname,fld_clname,fld_cemail,fld_cphone,fld_caddress,fld_ccountry,fld_cstate,fld_ccity,fld_czipcode,fld_dob,fld_ssn,fld_status,fld_active,fld_admin_per,fld_dist_per,fld_rep_per,fld_last_updated) VALUES(:fld_uid,:fld_cname,:fld_clname,:fld_cemail,:fld_cphone,:fld_caddress,:fld_ccountry,:fld_cstate,:fld_ccity,:fld_czipcode,:fld_dob,:fld_ssn,'1','1','$comm_admin','$comm_dist','$comm_rep',CURRENT_TIMESTAMP)";
			// echo "1760-".$sql."<br/>";
			$stmt = $this->db->prepare($sql);
			...
			// echo "<br/><br/>1775-debugDumpParams-". var_dump($stmt->debugDumpParams() ); echo "<br/><br/>";
			$stmt->execute();
	...
	public function hierarchy_campaign_details($uid) {
		try {
			$sql = "SELECT c.*, u.fld_cname AS fld_company
			FROM tbl_campaign c
	...
	public function send_email_to_donors($cid, $pid) {
		try {
			...
			$stmt = $this->db->prepare($sql);
			// echo "<br/>send_email_to_donors q1=".$sql."<br/>";
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
				//Find Distributor ID
				$sql2 = "SELECT u.*
				...
				ORDER BY u.fld_uid ASC
				LIMIT 1";
				$stmt90 = $this->db->prepare($sql2);
				// echo "<br/>send_email_to_donors q2=".$sql2."<br/>";
				$stmt90->execute();
				if ($stmt90->rowCount() > 0) {
					$refRow = $stmt90->fetch(PDO::FETCH_ASSOC);
					$ref_name = $refRow['fld_name'];
				...
				while ($email_manager_Row = $stmt->fetch(PDO::FETCH_ASSOC)) {
					...
					try {
						...
						$promise = $sparky->transmissions->post($sparkPostPostData1);
						// echo '2976-sparkPostPostData1<pre>'; print_r($sparkPostPostData1);echo '</pre>';
						$transmissionid = $promise->getBody()['results']['id'];
						$sql3 = "UPDATE tbl_donors_details SET sent_id = '$transmissionid', sent_email = '1', sent_date = NOW() WHERE cid = '$cid' AND puid = '$pid' AND uid = '$donorid'";
						// echo "<br/>sql3==".$sql3."<br/>";
						$stmt55 = $this->db->prepare($sql3);
						$stmt55->execute();
					} catch (\Exception $e) {
						echo $e->getCode() . "\n";
						echo $e->getMessage() . "\n";
					...
					if ($fld_text_messaging == 1 && $sms_sent == true) {
						$generate_short_link = sHOME.'l.php?v='.$fld_campaign_hashkey.'&u='.$pid.'&d='.$donorid.'';
	...
	public function sendstartyourcampaign($fld_name, $fld_organization, $fld_email, $fld_phone, $fld_call, $sn_dist_email, $getemail, $getname) {
		try {
			if ($sn_dist_email == 1) {
				...
				$email_name = SITE_TITLE_WITH_ADMIN;
	...
	public function participants_details2($pid, $cid) {
		try {
			$sql = "SELECT b.fld_campaign_id, b.fld_campaign_title, b.fld_cname, b.fld_donor_size,
				(SELECT COUNT(c.id) FROM tbl_donors_details c WHERE c.cid = a.cid AND c.puid = '$pid') AS donoruploaded,
				(SELECT IFNULL(SUM(d.donation_amount),0) FROM tbl_donations d INNER JOIN tbl_donors_details e ON e.uid = d.uid AND e.cid = d.cid WHERE e.puid = '$pid' AND d.mode = '1') AS moneyraised
				FROM tbl_participants_details a
				INNER JOIN tbl_campaign b ON b.fld_campaign_id = a.cid
	...
	public function donors_donated($did) {
		try {
			$sql = "SELECT a.ufname AS donationfname, a.ulname AS donationlname, a.uemail AS donationemail, c.fld_cname, c.fld_campaign_title, d.uname AS pname, d.ulname AS plname, d.uemail AS pemail, b.uname, b.ulname, b.uemail, b.uphone, a.donation_amount, a.id AS transactionno, DATE_FORMAT(a.creationdate, '%m/%d/%Y') AS donationdate
			FROM tbl_donations a
	...
	public function test1() {
		$refimage = "_gr1uIoUMmbxa3CvSTiwcO4ZAGQDKj.jpga";
		$refImgPath = dirname(__FILE__).'/../cms/uploads/brandlogo/'.$refimage;
		if( !file_exists($refImgPath) ){
			$refimage = "app_log.png";
		} 
						var_dump([$refImgPath, $refimage]); die();
	}
			...
			if ($stmt->rowCount() > 0) {
				...
				while ($email_manager_Row = $stmt->fetch(PDO::FETCH_ASSOC)) {
					...
					try {
						...
						//SparkPost Init
						$sparky->setOptions(['async' => false]);
						$Campaign_Title = isset($Campaign_Title) ? $Campaign_Title : "";
						$ParticipantFName = isset($ParticipantFName) ? $ParticipantFName : "";
						$ParticipantLName = isset($ParticipantLName) ? $ParticipantLName : "";

						// if refimage not exist then use application logo
						//app\cms\uploads\brandlogo
						$refImgPath = dirname(__FILE__).'/../cms/uploads/brandlogo/'.$refimage;
						if($refimage == ""){
							$refimage = "app_log.png";
						}
						else if( !file_exists($refImgPath) ){
							$refimage = "app_log.png";
						} 
						// echo '<pre>'; var_dump([$refImgPath, $refimage]);
						...
						// echo "post_data_to_sp=<pre>"; print_r($post_data_to_sp); die();
						$promise = $sparky->transmissions->post($post_data_to_sp);
						$transmissionid = $promise->getBody()['results']['id'];
					...
					if ($fld_text_messaging == 1) {
						$generate_short_link = sHOME.'l.php?v='.$fld_campaign_hashkey.'&u='.$uid.'&m=1';
						...
						if ($uphone != "" && $uphone != "000-000-0000" && $uphone != "___-___-____") {
							if ($sent_sms = send_sms(str_replace("-","",$uphone),utf8_encode($body))) {
								...
								if ($sms_sent_id != '') {
									$sms_date_created = $sent_sms['date_created'];
									$sms_message = isset($sent_sms['message']) ? $sent_sms['message'] : "";
									$sms_details = $sms_message;
					...
					}//end of if of fld_text_messaging == 1 //

					//Dont return it, othere wise it will send message to only one part from bulid team at the time of adding part.
					//return $email_manager_Row;
				}//end of while loop
			}//end of if of rowCount > 0
		} catch (PDOException $e) {
			echo $e->getMessage();
		}
	}//end of fn send_email_to_participants //
	...
	public function update_participant_details($cid,$cuid,$pid,$uphone) {
		try {
			...
			if ($stmt3->rowCount() > 0) {
				...
				if ($fld_text_messaging == 1) {
					$generate_short_link = sHOME.'l.php?v='.$campaign_hashkey.'&u='.$pid.'&m=1';
					$ParticipantFullName = trim($uname." ".$ulname);
					$body = "Hi! It is $ParticipantFullName.\n";
					$body = "You are being invited to join $ctitle by $cname. Please click on the link below to join this campaign.\n";
					$body .= "".$generate_short_link."\n";
	...
	public function update_donors_details($cid,$pid,$did,$uphone) {
		try {
			...
			if ($stmt3->rowCount() > 0) {
				...
				if ($fld_text_messaging == 1) {
					$generate_short_link = sHOME.'l.php?v='.$campaign_hashkey.'&u='.$participantid.'&d='.$did.'';
