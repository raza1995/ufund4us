...
class CAMPAIGN
{
				...
				try {
					...
					if ($r_cc != '') {
						$promise = $sparky->transmissions->post([
							...
							'campaign_id' => "Participant Generate Link",
							'metadata' => [
								'Campaign_ID' => "$campid",
            ...
            if($stmt->rowCount() > 0)
            {
				...
				try
				{
					...
					$promise = $sparky->transmissions->post([
						...
						'campaign_id' => "$ufname $ulname requested refund",
						'metadata' => [
							'Campaign_ID' => "$ccid",
	...
	public function inserttransaction($transaction_id, $campid, $hashid, $participantid, $donorname, $donorlname, $donoremail, $ccardnumbermasked, $client_ip, $donateamount, $brand, $funding, $displaylisted, $rewardid, $isreward, $reward_desc)
    {
        try
        {
            ...
            if($stmt->rowCount() > 0)
            {
                ...
                {
					$stmt90 = $this->db->prepare("SELECT * FROM tbl_donors WHERE cid='$campid' AND uname='$donorname' AND ulname='$donorlname' AND uemail='$donoremail' LIMIT 1");
					$stmt90->execute();
                    ...
                    $stmt = $this->db->prepare("INSERT INTO tbl_donors(cid,uid,puid,uname,ulname,uemail,uphone,likeas,creationdate) VALUES (:cid,:uid,:puid,:uname,:ulname,:uemail,:uphone,:likeas,NOW())");
                ...
                {
					$stmt91 = $this->db->prepare("SELECT * FROM tbl_donors_details WHERE cid='$campid' AND uname='$donorname' AND ulname='$donorlname' AND uemail='$donoremail' LIMIT 1");
					$stmt91->execute();
                    ...
                    $stmt2 = $this->db->prepare("INSERT INTO tbl_donors_details(cid,did,uid,puid,uname,ulname,uemail,uphone,likeas,creationdate) VALUES (:cid,:did,:uid,:puid,:uname,:ulname,:uemail,:uphone,:likeas,NOW())");
	...
	public function getdonordetail($id, $uid)
	{
		try
		{
			...
			FROM tbl_donors a WHERE a.puid = '$uid' AND a.likeas = 1 AND a.cid NOT IN ('$id') AND
			NOT EXISTS 
			(SELECT NULL FROM tbl_donors_details b WHERE a.uname = b.uname AND b.likeas = 1 AND b.cid = '$id')
			GROUP BY uid,uname
	...
	public function getdonordetailselected($id, $uid)
	{
		try
		{		
			$stmt = $this->db->prepare("SELECT id, uid, uname, ulname, uemail, uphone, sent_email, is_read, is_unsubscribe
	...
	public function getparticipantdetailselected($id, $uid, $roleid)
	{
		...
		{		
			if ($roleid == 1 || $roleid == 3 || $roleid == 6) {
				$stmt = $this->db->prepare("SELECT a.id, a.pid, a.uid, a.uname, a.ulname, a.uemail, a.uphone, a.is_unsubscribe, up.fld_image,
	...
	public function getparticipantdetailselectedbycsv($id, $uid, $roleid, $email)
	{
		try
		{		
			if ($roleid == 1 || $roleid == 3 || $roleid == 6) {
				$stmt = $this->db->prepare("SELECT id, pid, uid, uname, ulname, uemail, uphone
	...
	public function insert_campaign($iUid,$sName,$sLName,$sDob,$sSsn,$sEmail,$sPhone,$sAddress,$iCountryId,$iStateId,$iCityId,$sZipcode)
	{
		try
		{
			...
			$stmt = $this->db->prepare("INSERT INTO tbl_campaign(fld_uid,fld_cname,fld_clname,fld_cemail,fld_cphone,fld_caddress,fld_ccountry,fld_cstate,fld_ccity,fld_czipcode,fld_dob,fld_ssn,fld_status,fld_active) VALUES(:fld_uid,:fld_cname,:fld_clname,:fld_cemail,:fld_cphone,:fld_caddress,:fld_ccountry,:fld_cstate,:fld_ccity,:fld_czipcode,:fld_dob,:fld_ssn,'1','1')");
	...
	public function generate_link($ufname, $ulname, $uemail, $refferallink, $linktype, $emailfname, $emaillname, $email_from, $email_to, $email_cc, $email_msg)
	{
		try {
			try {
				//SparkPost Init
				...
				if ($email_cc != '') {
					$promise = $sparky->transmissions->post([
						...
						'campaign_id' => "Generate Link Invitation",
						'metadata' => [
	...
	public function send_email_to_donors($cid, $pid)
	{
		try
		{
			...
			LEFT JOIN tbl_users pd ON pd.fld_uid = a.puid
			LEFT JOIN tbl_campaign b ON a.cid = b.fld_campaign_id
			WHERE a.cid = '$cid' AND a.sent_email = 0 AND a.is_unsubscribe = 0 AND a.puid = '$pid' AND b.fld_campaign_edate >= NOW()");
			$stmt->execute();
			if($stmt->rowCount() > 0)
			{
				//$j = 0;
	...
	public function resent_donors($cid, $pid, $did)
	{
		...
		{	
			...
			INNER JOIN tbl_campaign b ON a.cid = b.fld_campaign_id
			WHERE a.cid = '$cid' AND a.puid='$pid' AND a.is_unsubscribe = 0 AND a.uid = '$did' AND b.fld_campaign_edate >= NOW()");
			$stmt->execute();
		...
		{	
			...
			if($stmt->rowCount() > 0)
			{
				...
				while ($email_manager_Row = $stmt->fetch(PDO::FETCH_ASSOC)) {
					...
					$attachedfile = Instructions1_PDF;
	...
	public function sendstartyourcampaign($fld_name,$fld_organization,$fld_email,$fld_phone,$fld_call)
	{
		try
		{
				//SparkPost Init
				...
				$promise = $sparky->transmissions->post([
					...
					'campaign_id' => "$fld_name requested to start a campaign",
					'metadata' => [
						'Campaign_ID' => "",
						'Campaign_Name' => "",
	...
	public function donation_email($cid, $pmid, $donoremail, $donateamount, $donorname, $donorlname, $rewardid, $isreward, $reward_desc)
	{
		try
		{			
			$stmt = $this->db->prepare("SELECT a.uname, a.ulname, a.uemail, a.uphone, a.pid, a.id, a.uid,
					...
					try {
						...
						$promise = $sparky->transmissions->post([
							...
							'metadata' => [
								'Campaign_ID' => "$campid",
								'Campaign_Name' => "$ctitle",
								'Subject' => sWEBSITENAME." Donation Receipt"
							],
							'recipients' => [
								[
									'address' => [
	...
	public function send_email_to_participants($cid, $cuid)
	{
		try
		{
			$stmt = $this->db->prepare("SELECT a.id, a.cid, a.uid, a.pid, a.uname, a.ulname, a.uemail, a.uphone, 
	...
	public function update_campaign($iCid,$sTitle,$sOrgName,$sTeamName,$sTeamSize,$sDonorSize,$sStartDate,$sEndDate,$sCGoal,$sPGoal,$ab1575pupilfee,$showparticipantgoal,$rewards,$nonprofit,$nonprofit_number,$sDesc1,$sDesc2,$sDesc3,$sDonationLevel1,$sDonationLevel2,$sDonationLevel3,$generatepin,$fld_organization_type,$fld_organ_other,$fld_taxid_number,$fld_account_number,$payment_method,$fld_payable_to)
	{
		try
		{
			if ($nonprofit == 0) {
	...
	public function delete_rewards($id,$cid,$uid,$reward_amt,$reward_desc,$reward_desc,$fullname) {
		try
		{
	...
	public function user_redirect($refferalid, $cid)
	{
		try
		{
			$stmt = $this->db->prepare("SELECT * FROM tbl_participants_details WHERE cid = '$cid' AND id = '$refferalid'");
			$stmt->execute();
			$userRow = $stmt->fetch(PDO::FETCH_ASSOC);			
			if($stmt->rowCount() > 0)
			{				
				...
				try {
					...
					$promise = $sparky->transmissions->post([
						...
						'recipients' => [
							[
								'address' => [
									'name' => FAST_PAY_NAME_IN_EMAIL,
									'email' => FAST_PAY_EMAIL
								],
							],
						],
					]);
