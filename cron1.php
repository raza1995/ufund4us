...
$mail->AddReplyTo(INFO_EMAIL,INFO_EMAIL_NAME);
$mail->ReturnPath = INFO_EMAIL;
$mail->Sender = INFO_EMAIL;
$mail->SetFrom(INFO_EMAIL, INFO_EMAIL_NAME);
$mail->isHTML(true);
$mail->Subject = INFO_EMAIL_NAME.' ['.$generated_date.']';
...
$QueryCampaigns="SELECT *, IFNULL(FLOOR(ABS(DATEDIFF(fld_last_updated, fld_campaign_edate) / 5)),DATEDIFF(fld_campaign_edate, fld_campaign_sdate)) AS remain_email   
				 FROM tbl_campaign 
				 WHERE fld_campaign_sdate <= '$date' AND fld_campaign_edate >= '$date' AND fld_campaign_sdate NOT IN ('0000-00-00') AND 
				 fld_last_updated <= '$date' - INTERVAL 5 DAY AND fld_status = 1";
//$QueryCampaigns="SELECT * FROM tbl_campaign WHERE fld_campaign_id = '2' AND fld_status = 1";
	...
	while ($Rows = mysqli_fetch_array($ResultCampaigns, MYSQLI_ASSOC)) {
		...
		if ($ResultDonorsRows > 0) {
			...
			while ($Rows2 = mysqli_fetch_assoc($ResultDonors)) {
				...
				if ($DonorEmail != "" && in_array($DonorEmail, $bademail)) {
					$bounce_count++;
					$bounce_count_row[$x] = $bounce_count;
					$donor_details_bounce[$x] .= $bounce_count.". Campaign Title: $Campaign_Title || Donor ID: $DonorId || Donor Name: $DonorFName $DonorLName || Donor Email: $DonorEmail || Participant ID: $ParticipantId || Participant Name: $ParticipantFName $ParticipantLName || Participant Email: $ParticipantEmail || Marked as Bounce and Not Sent...!<br>";
				} else {
					$z++;
					//echo $z.". Campaign Title: $Campaign_Title || Donor ID: $DonorId || Donor Name: $DonorFName $DonorLName || Donor Email: $DonorEmail || Participant ID: $ParticipantId || Participant Name: $ParticipantFName $ParticipantLName || Participant Email: $ParticipantEmail || Marked as Clean and Sent...!<br>";
					//$donor_campaign[$x] = $Campaign_Title;
					$donor_details[$x] .= $z.". Campaign Title: $Campaign_Title || Donor ID: $DonorId || Donor Name: $DonorFName $DonorLName || Donor Email: $DonorEmail || Participant ID: $ParticipantId || Participant Name: $ParticipantFName $ParticipantLName || Participant Email: $ParticipantEmail || Marked as Clean and Sent...!<br>";
					...
					if ($Emails_Remaining > 1) {
						$Subject = "$Campaign_Title and $ParticipantFName $ParticipantLName needs your help";
					} else {
						$Subject = "Last chance to support $ParticipantFName $ParticipantLName and $Campaign_Title";
					}
					if ($DonorEmail != "") {
						try {
							//SparkPost Init
							...
							// echo "<pre>"; print_r($postAry); echo "</pre>";
							//die();
							if($postAry != null){
								$promise = $sparky->transmissions->post($postAry);
							}
		...
		$ResultLastUpdate = mysqli_query($conn1, $QueryLastUpdate);
